...
// import {pathExistsSync, readFileSync, statSync, removeSync, writeFileSync, mkdirpSync} from 'fs-extra';
import pkg from 'fs-extra';
const {pathExistsSync, readFileSync, statSync, removeSync, writeFileSync, mkdirpSync} = pkg;
...
import map from '@joplin/lib/pluginCategories.json' assert {type: 'json'};
...
const userConfigPath = _resolve(rootDir, userConfigFilename);
const distDir = _resolve(rootDir, 'dist');
const srcDir = _resolve(rootDir, 'src');
const publishDir = _resolve(rootDir, 'publish');

const userConfig = {extraScripts: [], ...(pathExistsSync(userConfigPath) ? import(userConfigFilename, {with: {type: 'json'}}) : {})};

const manifestPath = `${srcDir}/manifest.json`;
const packageJsonPath = `${rootDir}/package.json`;
const allPossibleScreenshotsType = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
const manifest = readManifest(manifestPath);
const pluginArchiveFilePath = _resolve(publishDir, `${manifest.id}.jpl`);
const pluginInfoFilePath = _resolve(publishDir, `${manifest.id}.json`);
...
function validateCategories(categories) {
	...
	categories.forEach(category => {
		...
		}).includes(category)) throw new Error(`${category} is not a valid category. Please make sure that the category name is lowercase. Valid categories are: \n${map(category => {
			return category.name;
		})}\n`);
	});
}
...
function validateScreenshots(screenshots) {
	...
	for (const screenshot of screenshots) {
		...
		const screenshotPath = _resolve(rootDir, screenshot.src);
...
function createPluginArchive(sourceDir, destPath) {
	const distFiles = _sync(`${sourceDir}/**/*`, {nodir: true, windowsPathsNoEscape: true})
...
function onBuildCompleted() {
	try {
		removeSync(_resolve(publishDir, 'index.js'));
...
function resolveExtraScriptPath(name) {
	...
	const fullPath = _resolve(`${rootDir}/${relativePath}`);
	if (!pathExistsSync(fullPath)) throw new Error(`Could not find extra script: "${name}" at "${fullPath}"`);
